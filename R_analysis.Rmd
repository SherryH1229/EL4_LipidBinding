...
perc_count$Percentage <- (perc_count$Freq/sum(perc_count$Freq))*100
```
...
## Merge Batch
```{r}
EL4_count <- EL4_exon_count[,c(1,7:ncol(EL4_exon_count))] %>% column_to_rownames(.,"Geneid")
EL4_count_2 <- EL4_exon_count_2[,c(1,7:ncol(EL4_exon_count_2))] %>% column_to_rownames(.,"Geneid") %>% .[,-c(7,8)]

OutputVec_ENSEMBL <- c()
OutputVec_SYMBOL <- c()
for (i in seq(3, ncol(EL4_count), by=2)){
  temp_df <- cbind(EL4_count[c(i,i+1)],EL4_count_2[c(i,i+1)],EL4_count[c(1,2)],EL4_count_2[c(1,2)])
...
pp_bar_2 <- ggplot(pass_filter_count_df_2,aes(x = factor(Freq_thresh),y = Count))+
  geom_bar(stat="identity", fill="steelblue")+theme_minimal()+
  geom_text(aes(label=Count), vjust=-0.1, color="black", size=4.5)+
  xlab("Occur_Freq_thresh")+ggtitle("Count of Genes Across Different Num of Samples: Batch2")+
  theme(text = element_text(size = 18,face="bold"),
        axis.text.x = element_text(size = 18,face="bold",hjust=.1),
        axis.text.y = element_text(size = 18),
        plot.title = element_text(size=15,face="bold"),
...
# plot together
pass_filter_count_df$Batch <- "Batch_1"
pass_filter_count_df_2$Batch <- "Batch_2"

pass_df <- rbind(pass_filter_count_df,pass_filter_count_df_2)

pp_bar_3 <- ggplot(pass_df,aes(x = factor(Freq_thresh),y = Count,fill=Batch))+
...
pp_bar_3
ggsave("Bar_occur_all.png",pp_bar_3,height = 6 , width = 10)
```
...
merge_res <- merge(merge_res,anno_info_Mm,by = "SYMBOL")
write.table(merge_res,"High_occur_merge.csv",row.names = FALSE)
...
test <- Op_ENSEMBL[rowSums(is.na(Op_ENSEMBL)) != ncol(Op_ENSEMBL), ]
test2 <-Op_SYMBOL[rowSums(is.na(Op_SYMBOL)) != ncol(Op_SYMBOL), ]
write.table(test,"output_ENSEMBL",row.names = FALSE)
write.table(test2,"output_SYMBOL",row.names = FALSE)


Op_repOccur <- sapply(out_vec, '[', seq(max(sapply(out_vec, length)))) %>% 
  as.data.frame(.) %>% set_colnames(.,col_n)

write.table(Op_repOccur,"Rep_occur_AcrossBatch.csv",row.names = FALSE)
```
